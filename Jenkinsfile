pipeline {
    ...
    environment {
        ...
        GITOPS_BRANCH = "main"
        DEPLOYMENT_FILE = "react-multithread/multithread.deployment.yaml"
            ...
            steps {
                withCredentials([
                    ...
                    powershell script: '''
                        ...
                        (Get-Content -Path $manifest) -replace 'restartedAt: .*', "restartedAt: $timestamp" | Set-Content -Path $manifest

                        git -C $gitopsDir add $env:DEPLOYMENT_FILE

                        git -C $gitopsDir diff --cached --quiet; if ($LASTEXITCODE -ne 0) {
                            git -C $gitopsDir -c user.name=Jenkins -c user.email=jenkins@pipeline.com commit -m "react-multithread: atualiza imagem Docker para ${env:IMAGE_TAG}"
